...
describe('Flow', () => {
	...
	test('8 view Stardew Valley', async done => {
		const url = 'https://steamcdn-a.akamaihd.net/steam/apps/413150/library_600x900_2x.jpg'
		const stardewThumbnail = (await page.$x(`//*[@src="${url}"]`))[0]
		await Promise.all([
			page.waitForNavigation({ waitUntil: 'load' }),
	...
	test('17 preview positive long review', async done => {
		...
		await page.type('textarea[id=longReviewBody]', '![an image](https://via.placeholder.com/150 =250x250 "title")')
