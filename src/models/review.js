...
function hasPrivileges(visibility, privileges, username, author) {
  ...
  if (visibility === 'author' && username !== author) {
    throw Error(ErrorEnum.REVIEW_INSUFFICIENT_PRIVILEGES)
  }
  if (privileges === 'none') {
    return Error(ErrorEnum.REVIEW_INSUFFICIENT_PRIVILEGES)
  }
}
...
class Review {
  async load(author, data) {
    ...
    if (results && results.length !== 0) {
      await connection.run('update.review', this.rating, this.body, this.visibility, this.game, this.author, this.type)
    } else {
  ...
  async visibility(data, username, privileges) {
    ...
    if (results && results.length !== 0) {
      hasPrivileges(results[0].visibility , privileges, username, data.author)
